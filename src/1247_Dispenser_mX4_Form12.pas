unit Dispenser_mX4_with_Form12;

//https://github.com/MakeMagazinDE/Solder-Paste-Dispenser/tree/main
//https://github.com/breitsch2/Solder-Paste-Dispenser/tree/main
//#TODO: comport inisettings fix  - fixed with add section!
//#TODO: load aperture file as csv - open for V13
//#TODO: add submenu in Grid and check timerdraw - checked
...
begin
  ...
  Inc2(Result.X, parent.Left);
  Inc2(Result.Y, parent.Top);
end;
...
function TControlClientToScreen2(const Point: TPoint; afrm: Tform): TPoint;
var
  Origin: TPoint;
begin
  Origin := afrm.ClientOrigin;
  Result.X := Point.X + Origin.X;
  Result.Y := Point.Y + Origin.Y;     
end;

function TControlClientToScreen3(const Point: TPoint; agrd: TStringgrid): TPoint;
var
  Origin: TPoint;
begin
  //Origin := agrd.ClientOrigin;
  Result.X := Point.X + Origin.X;
  Result.Y := Point.Y + Origin.Y;
end;
  ...
  with Form1 do begin
    ...
    for aRow:= 1 to {Form1.}SgBlocks.RowCount - 1 do begin
      ...
      if SgBlocks.Cells[6, arow] = 'Circle' then
        GerberBitmap.Canvas.Ellipse(bm_scroll.x + ix - rx, bm_scroll.y + iy - ry, 
                                                 bm_scroll.x + ix + rx, bm_scroll.y + iy + ry)
      else
        GerberBitmap.Canvas.FillRect(rect(bm_scroll.x + ix - rx, bm_scroll.y + iy - ry, 
                                                 bm_scroll.x + ix + rx, bm_scroll.y + iy + ry));
    end;
...
begin
  ...
  writeln('debug pitch: '+flots(pitchy));
  grow:= 0;
  dia:= minFloat(pitchX, pitchY) * 0.6;
  writeln('debug pitch: '+flots(dia));
...
begin
  ...
  timer1.Free;
  processmessagesON;
  writeln('debug: procmessON, timer freed and dspenser form closed ');
...
begin
  ...
  TForm(sender).icon.loadfromresourcename(hinstance, 'XNETWORK');
  SgBlocks.Cells[0, 0]:= '#';
  ...
  try
    ...
    EditDwellTime.Text:= Ini.ReadString('Params', 'DwellTime', '230');
...
begin
  ...
  if (ssRight in Shift) and (arow > 0) then begin
    ...
    //pt:= TControlClientToScreen2(pt, sgApert);
    pt:= TWinControl(SgApert).clienttoscreen(pt);
...
begin
  ...
  // Context-Menu
  //writeln('debug - popup '+itoa(grow));
  if (ssRight in Shift) and (gRow > 0) then begin
    SgBlocks.Row:= grow;
    //writeln('debug - popup in '+itoa(grow));
    pt.x := X + 15;
    pt.y := Y - 10;
    pt := Tcontrol(SgBlocks).ClientToScreen(pt);  // fixed
...
begin
   ...
   //Exception: Control '' has no parent window.
       parent:= frm;
       ClientToScreen(frm.clientorigin)
       free;
   end; 
   ...
   //Exception: Control '' has no parent window.
       parent:= frm;
       ClientToScreen(frm.clientorigin)
       writeln('helpcontext '+itoa(helpcontext));
       free;
   end; 
   with tcustomcontrol.create(self) do begin
   //Exception: Control '' has no parent window.
       parent:= frm;
       ClientToScreen(frm.clientorigin)
       writeln('helpcontext custom'+itoa(helpcontext));
       free;
   end; 
   //with CL.AddClass(CL.FindClass('TCustomControl'),TCustomGrid) do begin
   with TCustomGrid.create(self) do begin
   //Exception: Control '' has no parent window.
       //tcustomcontrol(parent):= frm;
       writeln(botostr(tabstop));
       //mousecoord
       //canvas
       //ClientToScreen(frm.clientorigin);
       //writeln('graphix color'+itoa(color));
       free;
   end; 
   ...
   {  TStringGrid - property Cells[ACol, ARow: Integer]: string read GetCells write SetCells;
    property Cols[Index: Integer]: TStrings read GetCols write SetCols;
    property Objects[ACol, ARow: Integer]: TObject read GetObjects write SetObjects;
    property Rows[Index: Integer]: TStrings read GetRows write SetRows;    }
   with tgraphiccontrol.create(self) do begin
   //Exception: Control '' has no parent window.
       parent:= frm;
       ClientToScreen(frm.clientorigin)
       writeln('graphix color'+itoa(color));
   ...
   with TMyStringGrid.create(self) do begin
   //Exception: Control '' has no parent window.
       parent:= frm;
       ClientToScreen(frm.clientorigin)
       writeln('graphix color'+itoa(color));
...
procedure loadDisepenserForm;
begin
Form1:= TForm1.create(self);
with form1 do begin
  Left:= 0
  Top:= 0
  Caption:= 'PasteDispenser 0.94 by cm 8/22 by maXbox4 script'
  ClientHeight:= 639
  ClientWidth:= 884
  Color:= clBtnFace
  ...
  LabelStatus:= TLabel.create(form1)
  ...
  LabelXM:= TLabel.create(form1)
  with labelXM do begin
   parent:= form1
    Left:= 8
    Top:= 74
    Width:= 20
  ...
  LabelYM:= TLabel.create(form1)
  with labelYM do begin
   parent:= form1
    Left:= 8
    Top:= 99
    Width:= 20
  ...
  LabelZM:= TLabel.create(form1)
  with labelZM do begin
   parent:= form1
    Left:= 8
    Top:= 124
    Width:= 20
  ...
  LabelXW:= TLabel.create(form1)
  with labelXW do begin
   parent:= form1
    Left:= 76
    Top:= 74
    Width:= 20
  ...
  LabelYW:= TLabel.create(form1)
  with labelYW do begin
   parent:= form1
    Left:= 76
    Top:= 99
    Width:= 20
  ...
  LabelZW:= TLabel.create(form1)
  with labelZW do begin
   parent:= form1
    Left:= 76
    Top:= 124
    Width:= 20
  ...
  Bevel10:= TBevel.create(form1)
  with bevel10 do begin
   parent:= form1;
    Left:= 8
    Top:= 151
    Width:= 275
 ...
 Label5:= TLabel.create(form1)
  ...
  DrawingBox:= TPaintBox.create(form1)
  with drawingbox do begin
   parent:= form1;
    ...
    //ExplicitHeight:= 294   read-only property
  end ;
  ...
  Label19:= TLabel.create(form1)
  with label19 do begin
   parent:= form1
    Left:= 209
    Top:= 55
    Width:= 58
  ...
  with amemo1 do begin
   parent:= form1;
    ...
    ParentFont:= False
    TabOrder:= 0
    text:= 'G4 PO ok'+CRLF+'G91 G0 Y10.00 ok'+CRLF+'G4 P0 ok'+
                      CRLF+'G91 G0 Z10.00 ok'+CRLF+'G4 P0 ok'+CRLF+'G90 ok';
    //text:= 'G4 PO ok'+LF
   end ;
  Button_Settings:= TButton.create(form1)
  with Button_Settings do begin
   parent:= form1
    Left:= 74
    Top:= 6
    Width:= 50
  ...
  Button_open:= TButton.create(form1)
  with Button_open do begin
   parent:= form1
    Left:= 6
    Top:= 6
    Width:= 62
  ...
  EditStatus:= TEdit.create(form1)
  with editstatus do begin
   parent:= form1
    Left:= 134
    Top:= 8
    Width:= 343
   ...
   BitBtnXzero:= TBitBtn.create(form1)
   with bitbtnxzero do begin
    parent:= form1;
    Left:= 134
    Top:= 70
    Width:= 65
  ...
  BitBtnYzero:= TBitBtn.create(form1)
   with bitbtnyzero do begin
    parent:= form1;
    Left:= 134
    Top:= 95
    Width:= 65
  ...
  BitBtnZzero:= TBitBtn.create(form1)
   with bitbtnzzero do begin
    parent:= form1;
    Left:= 134
    Top:= 120
    Width:= 65
  ...
  BitBtn9:= TBitBtn.create(form1)
  with bitbtn9 do begin
   parent:= form1;
    Tag:= 12
    Left:= 98
    Top:= 157
  ...
  BitBtn8:= TBitBtn.create(form1)
  with bitbtn8 do begin
   parent:= form1;
    Tag:= 11
    Left:= 98
    Top:= 185
  ...
  BitBtn7:= TBitBtn.create(form1)
  with bitbtn7 do begin
   parent:= form1;
    Tag:= 10
    Left:= 98
    Top:= 213
  ...
  BitBtn10:= TBitBtn.create(form1)
  with bitbtn10 do begin
   parent:= form1;
    Tag:= 13
    Left:= 98
    Top:= 263
  ...
  BitBtn11:= TBitBtn.create(form1)
  with bitbtn11 do begin
   parent:= form1;
    Tag:= 14
    Left:= 98
    Top:= 290
  ...
  BitBtn12:= TBitBtn.create(form1)
  with bitbtn12 do begin
   parent:= form1;
    Tag:= 15
    Left:= 98
    Top:= 317
  ...
  BitBtn4:= TBitBtn.create(form1)
  with bitbtn4 do begin
   parent:= form1;
    Left:= 124
    Top:= 238
    Width:= 26
  ...
  BitBtn5:= TBitBtn.create(form1)
  with bitbtn5 do begin
   parent:= form1;
    Tag:= 1
    Left:= 152
    Top:= 238
  ...
  BitBtn6:= TBitBtn.create(form1)
  with bitbtn6 do begin
   parent:= form1;
    Tag:= 2
    Left:= 181
    Top:= 238
  ...
  BitBtn14:= TBitBtn.create(form1)
  with bitbtn14 do begin
   parent:= form1;
    Tag:= 4
    Left:= 44
    Top:= 238
  ...
  BitBtn15:= TBitBtn.create(form1)
  with bitbtn15 do begin
   parent:= form1;
    Tag:= 5
    Left:= 16
    Top:= 238
  ...
  BitBtn16:= TBitBtn.create(form1)
  with bitbtn16 do begin
   parent:= form1;
    Tag:= 22
    Left:= 248
    Top:= 157
  ...
  BitBtn17:= TBitBtn.create(form1)
  with bitbtn17 do begin
   parent:= form1;
    Tag:= 21
    Left:= 248
    Top:= 185
  ...
  BitBtn18:= TBitBtn.create(form1)
  with bitbtn18 do begin
   parent:= form1;
    Tag:= 20
    Left:= 248
    Top:= 213
 ...
 BitBtn19:= TBitBtn.create(form1)
  with bitbtn19 do begin
   parent:= form1;
    Tag:= 23
    Left:= 248
    Top:= 263
  ...
  BitBtn20:= TBitBtn.create(form1)
  with bitbtn20 do begin
   parent:= form1;
    Tag:= 24
    Left:= 248
    Top:= 290
  ...
  BitBtn21:= TBitBtn.create(form1)
  with bitbtn21 do begin
   parent:= form1;
    Tag:= 25
    Left:= 248
    Top:= 317
  ...
  BitBtnhome:= TBitBtn.create(form1)
  with bitbtnhome do begin
   parent:= form1;
    Left:= 332
    Top:= 95
    Width:= 70
  ...
  BitBtngotoZero:= TBitBtn.create(form1)
  with bitbtngotoZero do begin
   parent:= form1;
    Left:= 332
    Top:= 120
    Width:= 70
  ...
  EditPitchX:= TEdit.create(form1)
  with editpitchX do begin
   parent:= form1
    Left:= 410
    Top:= 159
    Width:= 41
  ...
  EditPadsX:= TEdit.create(form1)
  with editpadsX do begin
   parent:= form1
    Left:= 306
    Top:= 159
    Width:= 41
  ...
  EditPadsY:= TEdit.create(form1)
  with editpadsy do begin
   parent:= form1
    Left:= 306
    Top:= 186
    Width:= 41
  ...
  BitBtnCreateBGA:= TBitBtn.create(form1)
  with bitbtncreatebga do begin
   parent:= form1;
    Left:= 318
    Top:= 213
    Width:= 149
  ...
  BitBtnAbort:= TBitBtn.create(form1)
  with bitbtnabort do begin
   parent:= form1;
    Left:= 332
    Top:= 70
    Width:= 70
  ...
  BitBtn23:= TBitBtn.create(form1)
  with bitbtn23 do begin
   parent:= form1;
    Left:= 712
    Top:= 329
    Width:= 164
  ...
  SgBlocks:= TStringGrid.create(form1)
 ...
 // object SgApert: TStringGrid
  SgApert:= TStringGrid.create(form1)
  with SgApert do begin
   parent:= form1
    Tag:= 3
    Left:= 680
    Top:= 30
  ...
  BitBtnRun:= TBitBtn.create(form1)
  with bitbtnrun do begin
   parent:= form1;
    Left:= 332
    Top:= 45
    Width:= 70
   ...
   EditPitchY:= TEdit.create(form1)
  with editpitchy do begin
   parent:= form1
    Left:= 410
    Top:= 186
    Width:= 41
  ...
  //object EditDwellTime: TEdit
   EditDwellTime:= TEdit.create(form1)
  with editDwellTime do begin
   parent:= form1
    Left:= 410
    Top:= 72
    Width:= 41
  ...
  EditPadTime:= TEdit.create(form1)
  with editPadTime do begin
   parent:= form1
    Left:= 410
    Top:= 97
    Width:= 41
  ...
  EditSavedX:= TEdit.create(form1)
  with editsavedx do begin
   parent:= form1
    Left:= 209
    Top:= 72
    Width:= 62
  ...
  EditSavedY:= TEdit.create(form1)
  with editsavedy do begin
   parent:= form1
    Left:= 209
    Top:= 97
    Width:= 62
  ...
  EditSavedZ:= TEdit.create(form1)
  with editsavedz do begin
   parent:= form1
    Left:= 209
    Top:= 122
    Width:= 62
  ...
  //object BitBtnGotoPos: TBitBtn
   BitBtnGotopos:= TBitBtn.create(form1)
  with bitbtngotopos do begin
   parent:= form1;
    Left:= 277
    Top:= 95
    Width:= 44
   ...
   BitBtnsavepos:= TBitBtn.create(form1)
  with bitbtnsavepos do begin
   parent:= form1;
    Left:= 277
    Top:= 70
    Width:= 44
   ...
   BitBtnAllzero:= TBitBtn.create(form1)
  with bitbtnallzero do begin
   parent:= form1;
    Left:= 134
    Top:= 45
    Width:= 65
   ...
   BitBtnFeedair:= TBitBtn.create(form1)
  with bitbtnfeedair do begin
   parent:= form1;
    Left:= 157
    Top:= 290
    Width:= 65
  ...
  BtnSO8:= TButton.create(form1)
  with btnso8 do begin
   parent:= form1
    Left:= 318
    Top:= 241
    Width:= 40
  ...
  BtnSO16w:= TButton.create(form1)
  with btnso16w do begin
   parent:= form1
    Left:= 364
    Top:= 241
    Width:= 58
  ...
  BtnBGA:= TButton.create(form1)
  with btnbga do begin
   parent:= form1
    Left:= 427
    Top:= 241
    Width:= 40
 ...
 // object EditPadTimeRO: TEdit
  EditPadTimeRO:= TEdit.create(form1)
  with EditPadTimeRO do begin
   parent:= form1
    Left:= 410
    Top:= 122
    Width:= 41
  ...
  //object PopupMenu1: TPopupMenu         to do 
  PopupMenu1:= TPopupMenu.create(self)
  PopupMenu2:= TPopupMenu.create(self)
    //Left:= 616
    //Top:= 424
    //object SetReferenceNozzleisatthisPad1: TMenuItem
    SetReferenceNozzleisatthisPad1:= TMenuItem.create(popupmenu1)
    with SetReferenceNozzleisatthisPad1 do begin
      popupmenu1.Items.Add(SetReferenceNozzleisatthisPad1);
      Caption:= 'Set Zero Reference (Nozzle is at this Pad)'
      OnClick:= @SetZeroReferencePoint;
    end ;
    movetothispadXY:= TMenuItem.create(popupmenu1)
    with  movetothispadXY do begin
      popupmenu1.Items.Add(movetothispadXY);
      Caption:= 'Move to this Pad (XY)'
      OnClick:= @MovetothispadXYClick ;
    end;
    //DispensethisPad1: TMenuItem
    DispensethisPad1:= TMenuItem.create(popupmenu1)
    with  DispensethisPad1 do begin
      popupmenu1.Items.Add(DispensethisPad1);
    ...
    EnableAperture1:= TMenuItem.create(popupmenu1)
    with  EnableAperture1 do begin
      popupmenu1.Items.Add(EnableAperture1);
      Caption:= 'Circle'
      OnClick:= @SgBlocksCircleClick
    end;
    //object DisableAperture1: TMenuItem
    DisableAperture1:= TMenuItem.create(popupmenu1)
    with  DisableAperture1 do begin
      popupmenu1.Items.Add(DisableAperture1);
      Caption:= 'Rectangle'
      OnClick:= @SgBlocksRectClick
    end;
    //object DisableallApertures1: TMenuItem
    DisableallApertures1:= TMenuItem.create(popupmenu1)
    with  DisableallApertures1 do begin
      popupmenu1.Items.Add(DisableallApertures1);
      Caption:= 'None (OFF)'
...
(*  end  *)
  //object PopupMenu2: TPopupMenu
    ...
    DispensePadswiththisAperture1:= TMenuItem.create(popupmenu2)
    with DispensePadswiththisAperture1 do begin
      popupmenu2.Items.Add(DispensePadswiththisAperture1);  
      Caption:= 'Dispense Pads with this Aperture'
      OnClick:= @DispensePadswiththisAperture1Click
    end ;
    //object N3: TMenuItem
    N3:= TMenuItem.create(popupmenu2)
    with  N3 do begin
      popupmenu2.Items.Add(N3);
      Caption:= '-'
    end ;
    //object MenuItemApertCircle: TMenuItem
    MenuItemApertCircle:= TMenuItem.create(popupmenu2)
    with MenuItemApertCircle do begin
      popupmenu2.Items.Add(MenuItemApertCircle);  
      Caption:= 'Circle'
      OnClick:= @MenuItemApertCircleClick
    end ;
    //object MenuItemApertRect: TMenuItem
    MenuItemApertRect:= TMenuItem.create(popupmenu2)
    with MenuItemApertRect do begin
      popupmenu2.Items.Add(MenuItemApertRect);
      Caption:= 'Rectangle'
      OnClick:= @MenuItemApertRectClick
    end ;
    //object MenuItemApertOff: TMenuItem
    MenuItemApertOff:= TMenuItem.create(popupmenu2)
    with MenuItemApertOff do begin
      popupmenu2.Items.Add(MenuItemApertOff);
      Caption:= 'None (OFF)'
      OnClick:= @MenuItemApertOffClick
    end;
    N2:= TMenuItem.create(popupmenu2)
    with  N2 do begin
      popupmenu2.Items.Add(N2);
      Caption:= '-'
    end ;
    //object MenuItemApertDisableAll: TMenuItem
    MenuItemApertDisableAll:= TMenuItem.create(popupmenu2)
    with MenuItemApertDisableAll do begin
      popupmenu2.Items.Add(MenuItemApertDisableAll);
      Caption:= 'Disable All'
      OnClick:= @MenuItemApertDisableAllClick
    end;
    //object MenuItemApertEnableAll: TMenuItem
    MenuItemApertEnableAll:= TMenuItem.create(popupmenu2)
    with MenuItemApertEnableAll do begin
      popupmenu2.Items.Add(MenuItemApertEnableAll);
      Caption:= 'Enable All'
      OnClick:= @MenuItemApertEnableAllClick
    end;
  //end  *)
  TimerDraw:= TTimer.create(self)
  with timerdraw do begin
...
ref :  TCustomControl = class(TWinControl)
  private
    FCanvas: TCanvas;
...
//{$ENDIF}
  ...
  GitGuardian has detected the following RSA Private Key exposed within your GitHub account.

Details

- Secret type: RSA Private Key

- Repository: maxkleiner/maXbox4

- Pushed date: August 22nd 2023, 09:14:01 UTC
