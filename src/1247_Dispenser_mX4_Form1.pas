...
//#TODO: load aperture file as csv
//#TODO: add submenu in Grid and check timerdraw
...
begin
  ...
  if (ssRight in Shift) and (arow > 0) then begin
    ...
    pt:= TControlClientToScreen2(pt, form1);
...
begin
  ...
  if (ssRight in Shift) and (gRow > 0) then begin
    ...
    pt.x := X + 15;
    pt.y := Y - 10;
    pt := Tcontrol(SgBlocks).ClientToScreen(pt);  // fix
...
with form1 do begin
  Left:= 0
  Top:= 0
  Caption:= 'PasteDispenser 0.92 by cm 8/22 maXbox4 script'
  ClientHeight:= 639
  ClientWidth:= 884
  Color:= clBtnFace
